...
def xls_inject_cbd_single(project, part, vendor, workbook):
    ...
    # part general info
    part_general_info = sql.get_part_general_info(project, part)
    if part_general_info:
    ...
    # vendor info
    sheet['L7'] = vendor
    vendor_info = sql.get_vendor_info(vendor)
    if vendor_info:
    ...
    # save the inject
    output_file_name = './output/' + 'CBD' + '_' + project + '_' + part + '_' + vendor + '_000' + '.xlsx'
    ws = workbook.active
...
def xls_inject_ss_single(project, part, wb):
    ...
    # lifetime:
    lifetime = sql.get_part_lifetime(project, part)
    if lifetime:  # use if to avoid lifetime=0
        sheet['K12'] = lifetime

    # volume_average
    vol_avg = sql.get_part_volume_avg(project, part)
    if vol_avg:
        sheet['E14'] = int(vol_avg)

    # PVO
    pvo = sql.get_part_target_pvo_part(project, part)
    if pvo:
        sheet['K14'] = pvo

    # part_timing
    part_timing = sql.get_part_timing(project, part)
    if part_timing:
    ...
    # save the inject
    output_file_name = './output/' + 'supplier_selection' + '_' + part + '.xlsx'
...
def xls_inject_sb(project, part_list):
    ...
    for part in part_list:
        ...
        if part_general_info and bom_part_row_i < 4:
            ...
            ## target_PVO
            target_pvo_total = sql.get_part_target_pvo_total(project, part)
            if target_pvo_total:
                sheet.cell(row=bom_part_row_num,
