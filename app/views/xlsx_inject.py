...
def xls_inject_risk_eval(project, part_list):
    ...
    for part in part_list:
        ...
        # part general info
        part_general_info = sql_query.get_part_general_info(*context)
        ...
        # volume_avg
        vol_avg = sql_query.get_part_volume_avg(*context)
        if vol_avg:
            sheet.cell(row=7, column=column).value = int(vol_avg)

        # target price:
        target_price = sql_query.get_part_target_price_avg_100EUR(*context)
        sheet.cell(row=8,
                   column=column).value = target_price  # Input as None is OK

        # lifetime:
        lifetime = sql_query.get_part_lifetime(*context)
        if lifetime:  # avoid lifetime=0
            sheet.cell(row=9, column=column).value = lifetime

        # part_timing
        part_timing = sql_query.get_part_timing(*context)
        if part_timing:
...
def xls_inject_cbd_single(project, part, vendor, workbook):
    ...
    # part general info
    part_general_info = sql_query.get_part_general_info(project, part)
    if part_general_info:
    ...
    # vendor info
    sheet['L7'] = vendor
    vendor_info = sql_query.get_vendor_info(vendor)
    if vendor_info:
    ...
    # yearly volume
    volume_dict = sql_query.get_part_volume_yearly(project, part)
    if volume_dict:
...
def xls_inject_cbd_project(project, part_list):
    ...
    for part in part_list:
        # prepare vendor list
        vendor_list = sql_query.get_vendor_list_under_part(project, part)
...
def xls_inject_ss_single(project, part, wb):
    ...
    # lifetime:
    lifetime = sql_query.get_part_lifetime(project, part)
    if lifetime:  # use if to avoid lifetime=0
        sheet['K12'] = lifetime

    # volume_average
    vol_avg = sql_query.get_part_volume_avg(project, part)
    if vol_avg:
        sheet['E14'] = int(vol_avg)

    # PVO
    pvo = sql_query.get_part_target_pvo_part(project, part)
    if pvo:
        sheet['K14'] = pvo

    # part_timing
    part_timing = sql_query.get_part_timing(project, part)
    if part_timing:
...
def xls_inject_sb(project, part_list):
    ...
    for part in part_list:
        ...
        if part_general_info and bom_part_row_i < 4:
            ...
            ## target_PVO
            target_pvo_total = sql_query.get_part_target_pvo_total(project, part)
            if target_pvo_total:
                sheet.cell(row=bom_part_row_num,
